...
def details(service_id: str) -> None:  # pragma: no cover
    ...
    data = {
        ...
        "connection": {
            ...
            "port": service.port,
            "mount": service.mount,
            # Read from the response body, not the reconstructed service:
            # `protected` is derived from the API key, which the API never
            # returns, so the rebuilt row always reports False.
